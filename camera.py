...
#pathf='F:\\python\\anaconda\\envs\\tensorflow\\Lib\\site-packages\\haarcascade_eye.xml'
class Camera_reader(object):
    #在初始化camera的时候建立模型，并加载已经训练好的模型
 def predict(self, img):
        img = img.astype('float32')
        #img = img/255.0
        img = img.reshape((-1, 30912))
        #img = np.reshape(img,[-1,112,91,3])
        y_=sess.run(op_to_predict, feed_dict={x:img})
        #max_index = np.argmax(y_,0)  # 找出概率最高的
        #max_index[0] #y[max_index]
 def build_camera(self):
    ...
    while success and cv2.waitKey(1) == -1:
        ...
        for (x, y, w, h) in faces:
            ...
            #grayimg1=cv2.cvtColor(ROI, cv2.COLOR_BGR2GRAY)
            ...
            if c == ord('q'):
                ...
                label = self.predict(img0)  # 利用模型对cv2识别出的人脸进行比对
...
if __name__ == '__main__':
   ...
   # model = os.path.abspath('model/')
    saver = tf.train.import_meta_graph('model1/my-gender-v1.0.meta')
